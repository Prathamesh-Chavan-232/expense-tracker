...
def list_all_expenses():
	...
	all_data = connector.execute('SELECT * FROM numbers')
...
def selected_expense_to_words():
	c = p2.connect_to_database()

	c.execute("SELECT * FROM numbers")
	result = c.fetchall()
	f = connector.execute(query = "Select sum(amnt) from ExpenseTracker")
	for i in f:
		for j in i:
			mb.showinfo('Current Balance: ', f"Total Expense: ' {j} \nBalance Remaining: {add.number-j}")
	print(result)
	# global table
...
Button(data_entry_frame, text='Total Balance', font=btn_font, width=30, bg=hlb_btn_bg).place(x=10,y=450)
